...
def split_train_test(df_for_split: pd.DataFrame, y_name: str = config.model_config.target, test_size: float = config.model_config.test_size, stratify_by: str = "sex") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                                                        ...
                                                        stratify=df_for_split[stratify_by]
                                                        )
...
def grid_search_hyperparameter_tuning(model_to_tune: GradientBoostingClassifier, X_train: pd.DataFrame, y_train: pd.DataFrame, search_grid: dict, cv: int = 3) -> GradientBoostingClassifier:
...
def evaluate_classification_and_log(classification_model: GradientBoostingClassifier, X_train: pd.DataFrame, y_train: pd.DataFrame, X_test: pd.DataFrame, y_test: pd.DataFrame):
...
if __name__ == "__main__":
    ...
    search_grid = {'learning_rate': [0.02, 0.05, 0.1],
